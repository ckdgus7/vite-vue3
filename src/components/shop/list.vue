        ...
        <div class="shop-content">
          ...
          <div id="sct">
            <transition-group name="list" tag="ul" class="sct sct_10 lists-row">
              <li
                v-for="(shopList, i) in GET_SHOPPING_LIST.list"
                :key="`${shopList.it_id}-${i}`"
                class="sct_li col-row-3 list-item"
                data-css="nocss"
                style="height: auto"
              >
                <div class="sct_img">
                  ...
                  <div class="sct_btn list-10-btn">
                    <button
                      type="button"
                      @click.prevent="addCartItem(shopList)"
                      class="btn_cart sct_cart"
                    >
...
export default {
  ...
  setup(props, ctx) {
    ...
    const { searchList } = useListFilter(
      actions.FETCH_SHOPPING[0],
      { ls: state.ls, la: state.la },
      ctx
    );
    const { showCartModal, addCartItem, closeCartModal } = useAddCart(
      ctx,
      actions
    );
    ...
    const loadMore = () => {
      ...
      if (state.la <= GET_SHOPPING_LIST.value.total_count) {
        setTimeout(() => {
          ...
          const ls = state.ls;
          const la = state.la;
          actions.FETCH_SHOPPING[0]({ kword, ls, la }).then(() => {
            busy.value = false;
          });
        }, 300);
      }
    ...
    actions.FETCH_SHOPPING[0]({ kword: "", ls: state.ls, la: state.la }).then(
      () => {
        busy.value = false;
      }
    );
